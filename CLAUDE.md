...
The metrics-band numbers (Publications / Citations / h-index / i10-index) refresh themselves via **GitHub Actions**, not a Claude routine: `.github/workflows/metrics-refresh.yml` (Mondays 16:07 UTC ≈ 9:07am PT, plus a Thursday retry, plus `workflow_dispatch` with a `dry_run` input) runs `.github/scripts/update_metrics.py`, which reads Citations/h-index/i10 from Google Scholar and Publications from the CV's `Publications (... N publications ...)` heading via `pdftotext`, rewrites the four tiles in `site/index.html`, and pushes to `master` (auto-deploys). Runs are idempotent — no change means no commit.

Two non-obvious things about it:
- **Google Scholar blocks datacenter IPs intermittently.** A direct fetch from a runner sometimes returns the real 175 KB page and sometimes a ~1 KB "We're sorry…" stub. Hence `SOURCES` (direct → AllOrigins → codetabs) retried over `ROUNDS=5` passes 45 s apart. `r.jina.ai` and `corsproxy.io` were tested and don't work. If everything is blocked the run **fails on purpose** so GitHub emails, rather than silently skipping a week.
